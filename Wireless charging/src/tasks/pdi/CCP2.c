 ...
 *the length of ccp2rcode must larger one byte than the SN: this one time, Max3232 5V is forget, 12V is forget too
 *SN is the ccp2rcode!!!!
 *add filter in usdt.c 
 *add gpio config in drv.c and usdt.c like CONFIG_PDI_ccp2_gujiamao 
 ...
 *optimize the ccp2rcode length, need change once ahead not as usual
 *no need to read the ccp2rcode by MCU, directly get by Up computer, so the uart2 is used for kline
 *jiamao.gu@2013 9.25
...
#define Length_barcode 8        // TBD for different Product
...
//pdi ccp2 can msg
static const can_msg_t ccp2_clrdtc_msg =		{ID, 8, {0x04, 0x31, 0x01, 0xF0, 0x05,0, 0, 0}, 0};
//static const can_msg_t ccp2_resis =			{0x607, 8, {0x03, 0x22, 0xfe, 0x80, 0, 0, 0, 0}, 0};
#if PDI_DEBUG
static const can_msg_t ccp2_errcode_msg =		{ID, 8, {0x03, 0x19, 0xfd, 0x39, 0, 0, 0, 0}, 0};
static const can_msg_t ccp2_getsn_msg =			{ID, 8, {0x03, 0x22, 0xfe, 0x8d, 0, 0, 0, 0}, 0};
static const can_msg_t ccp2_part_msg =			{0x7D2, 8, {0x03, 0x22, 0xF1, 0x00, 0x55, 0x55, 0x55, 0x55}, 0};
#endif
static const can_msg_t ccp2_reqseed_msg =		{ID, 8, {0x02, 0x27, 0x61, 0, 0, 0, 0, 0}, 0};
static const can_msg_t ccp2_start_msg =			{ID, 8, {0x02, 0x10, 0x03, 0, 0, 0, 0, 0}, 0};
static can_msg_t ccp2_msg_buf[32];		// for multi frame buffer
static char ccp2_data_buf[256];			// data buffer
static char ccp2_fault_buf[64];			// fault buffer
...
static char autoliv_buf[256];          	// for Xuce Up computer buffer					// for Xuce config file
...
// get data from ECU
static int ccp2_GetCID(char sid,short cid, char *data);
static int ccp2_GetFault(char *data, int *pnum_fault);
...
int pdi_startsession(void)
{
	...
	do {
		usdt_GetDiagFirstFrame(&ccp2_start_msg, 1, NULL, &msg, &msg_len);		//start session, why twice?	
		try_time_start --;
...
#if PDI_DEBUG
	printf("111111\r\n");
	can_msg_print(&ccp2_start_msg,"\n");
	can_msg_print(&msg, "\n");
#endif
	int try_time_reqseed = 5;
	do {
		usdt_GetDiagFirstFrame(&ccp2_reqseed_msg, 1, NULL, &msg, &msg_len);		//req seed
		try_time_reqseed --;
...
#if PDI_DEBUG
	printf("222222\r\n");
	can_msg_print(&ccp2_reqseed_msg,"\n");
	can_msg_print(&msg, "\n");
#endif
	...
	/*get serial number*/
	printf("\nSN Code:\n");
	usdt_GetDiagFirstFrame(&ccp2_getsn_msg, 1, NULL, ccp2_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(ccp2_msg_buf, msg_len);
	for (i = 0; i < msg_len; i ++)
		can_msg_print(ccp2_msg_buf + i, "\n");

	/*get error code*/
	printf("\nError Code:\n");
	usdt_GetDiagFirstFrame(&ccp2_errcode_msg, 1, NULL, ccp2_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(ccp2_msg_buf, msg_len);
	for (i = 0; i < msg_len; i++)
		can_msg_print(ccp2_msg_buf + i, "\n");
	/*tester point*/
	printf("\nPart:\n");
	usdt_GetDiagFirstFrame(&ccp2_getpart_msg, 1, NULL, ccp2_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(ccp2_msg_buf, msg_len);
	for (i = 0; i < msg_len; i++)
		can_msg_print(ccp2_msg_buf + i, "\n");

	if (ccp2_GetFault(ccp2_fault_buf, &num_fault))
		printf("##ERROR##\n");
	else {
		printf("##OK##\n");
		printf("num of fault is: %d\n", num_fault);
		for (i = 0; i < num_fault*2; i += 2)
			printf("0x%02x, 0x%02x\n", ccp2_fault_buf[i]&0xff, ccp2_fault_buf[i+1]&0xff);
	}
	/*clear all*/
	printf("\nClear all:\n");
	if (usdt_GetDiagFirstFrame(&ccp2_clrdtc_msg, 1, NULL, &msg, &msg_len))	//req seed
		return 1;
	can_msg_print(&msg, "\n");
#endif
#endif
...
static int ccp2_GetCID(char sid,short cid, char *data)
{
	...
	if (msg_len > 1) {
		ccp2_msg_buf[0] = msg_res;
		if(usdt_GetDiagLeftFrame(ccp2_msg_buf, msg_len))
			return 1;
	}
	if (msg_len == 1) {
		...
		data += 3;
		for (i = 1; i < msg_len; i++) {
			memcpy(data, (ccp2_msg_buf + i)->data + 1, 7);
			data += 7;
		}
	}
	return 0;
...
// return 1 : no date
// retrun 0 : ok
static int ccp2_read_barcode() 
{
	char read_bcode[Length_barcode + 1];
	int try_times = 5;
	while (ccp2_GetCID(0x22,0xfd3c, ccp2_data_buf)) {
		try_times --;
		if (try_times < 0)
			return 1;
	}
	memcpy(read_bcode, ccp2_data_buf, Length_barcode);
...
// return 0 : ok
static int ccp2_check_barcode()
{
	char read_bcode[Length_barcode + 1];
	int try_times = 5;
	printf("##START##EC-Checking ccp2rcode...##END##\n");
	while (ccp2_GetCID(0x22,0xfd3c, ccp2_data_buf)) {
		try_times --;
		if (try_times < 0)
			return 1;
	...
	printf("##END##\n");
	if(position == LEFT) {
		if (memcmp(read_bcode, bcodea, Length_barcode))  //maybe Length_ccp2rcode - 1 ??
			return 1;
	}
	else {
...
//return 1 : no date
//return 0 : ok
static int ccp2_read_sn()
{
	char read_sn[Length_SN + 1];
	int try_times = 5;
	memset(ccp2_data_buf,0,sizeof(ccp2_data_buf));
	while(ccp2_GetCID(0x22,0xfefc,ccp2_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	}
	ccp2_data_buf[Length_SN] = '\0';
	memcpy(read_sn,ccp2_data_buf,sizeof(read_sn));
	printf("SN:%s\r\n",read_sn);
	return 0;
}
/* equal Serial Number */
...
#define L 12
static int ccp2_read_softversion()
{
	char read_softversion[L + 1];  // 12 means length
	int try_times = 5;
	memset(ccp2_data_buf,0,sizeof(ccp2_data_buf));
	while(ccp2_GetCID(0x22,0xfd42,ccp2_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	...
	ccp2_data_buf[L] = '\0';
	memcpy(read_softversion,ccp2_data_buf,sizeof(read_softversion));
	printf("%s\r\n",read_softversion);
	return 0;
}
static int ccp2_check_softversion()
{
...
/*checking Resis*/
/*static int ccp2_check_resis()
{
	int msg_len;
	can_msg_t msg;
	printf("##START##EC-Checking resis...##END##\n");
	if (usdt_GetDiagFirstFrame(&ccp2_resis, 1, NULL, &msg, &msg_len))
		return 1;
	if (msg_len > 1)
	usdt_GetDiagLeftFrame(ccp2_msg_buf, msg_len);
...
int pdi_clear_dtc(void)
{
	...
	can_msg_t msg;
	pdi_startsession();
	if (usdt_GetDiagFirstFrame(&ccp2_clrdtc_msg, 1, NULL, &msg, &msg_len))
		return 1;
	if (msg.data[1] != (ccp2_clrdtc_msg.data[1] + 0x40))	// TBD By Me
		return 1;
	return 0;
}
...
{
	int i, result = 0;
	//if (ccp2_GetCID(0x19,0x020a, data))
	if (ccp2_GetCID(0x19,0x028b, data))
		return 1;
...
** compare ccp2rcode from ECU and scaner  **
...
ERROR_E Testbarcode(void)
{
	...
	for (rate = 5; rate <= 17; rate ++) {
		...
		printf("##START##STATUS-");		// in such format, so the data can be displayed in processccp2r
		sprintf(processbar, "%d", rate);
		printf("%s", processbar);
		printf("##END##\n");
	}
	pdi_startsession();                 // startsession before getting message
	if (ccp2_check_barcode()) {
...
ERROR_E TestECU(void)
{
	...
	memset(ccp2_fault_buf,0,sizeof(ccp2_fault_buf)); // clear this buffer, if not, you will see the bug
	int try_times = 0;
	int result = 0;
	int num_fault = 0;
	...
	*******************************************************************/
	pdi_startsession();
	do{
		result = ccp2_GetFault(ccp2_fault_buf,&num_fault);
		if(++try_times == 4) {
			error_result = ERROR_CW;
			return error_result;
		}
	...
	if(!num_fault) {
		...
		for (int i = 0; i < num_fault * 4; i += 4)  // according to size of error code , sometimes 3 bytes 
			printf("0x%02x, 0x%02x,0x%02x,0x%02x\n", ccp2_fault_buf[i]&0xff,ccp2_fault_buf[i+1]&0xff,ccp2_fault_buf[i+2]&0xff,ccp2_fault_buf[i+3]&0xff);
		printf("##END##\n");
		if(pdi_error_check(ccp2_fault_buf, &num_fault, autoliv)) { // check whether the error code is allowable
			...
			autoliv_send(&pdi_autoliv,"ERROR\r\n ");  //new protocol add by hyf at shanghai 2014/5/15  ' 'for the upper's convinence
				for(int i = 0; i < num_fault * 4; i++){   //send errorcode to up
					char a , b;
					a = ccp2_fault_buf[i] >> 4;
					b = ccp2_fault_buf[i] & 0x0f;
					if((a >= 0) && (a <= 9)) {
						a += '0';
					} else if((a >= 10) && (a <= 15)) {
						a = a - 10 + 'a';
...
{	
	...
	do {                                // if no extern CAN
		pdi_mdelay(1000);
	} while(!usdt_GetDiagFirstFrame(&ccp2_start_msg, 1, NULL, &msg, &msg_len));
}
/******************************
** change the status **********
** unlock the fixture *********
...
{
	memset(autoliv_buf,0,sizeof(autoliv_buf));   // Gloccp2l variable attention!!!
	time_t over_time;
...
void Step0(void)
{
	autoliv_send(&pdi_autoliv,"ECUPDIIDLT002\r\n");  // Send to Up
	printf("%s\r\n","ECUPDILT002");
}
/***********************************************************************************
...
// add by jiamao.gu in Shanghai 2013.10.28
// ccp2rcode sended after teststop in about three seconds, so need to wait 
...
void Receive_from_Up(time_t over_time)
{
	while(time_left(over_time) > 0) {
		if(SN[0] != 0) { // All the receive data put in autoliv_buf			
			printf("Receive other ccp2rcode and SN success !\r\n");
			return ;
		}
		sys_update();
	}
...
#if 1
static int cmd_ccp2_func(int argc, char *argv[])
{
	int num_fault, i;

	const char *usage = {
		"ccp2 , usage:\r\n"
		"ccp2 fault		read the error code\r\n"
		"ccp2 jama \r\n"
		"ccp2 target \r\n"
		"ccp2 start     test the ccp2 \r\n"
		"ccp2 sn          read the sn\r\n"
		"ccp2 barcode     read barcode inner\r\n"
		"ccp2 IMU    read IMU can message\r\n"
		"ccp2 clear       clear the history error\r\n"
		"ccp2 lock x      lock the fixture x\r\n"
		"ccp2 unlock x    unlock the fixture x\r\n"
		"ccp2 printf relay file \r\n"
		"ccp2 get position  \r\n"
		"ccp2 set position x  left or right\r\n"
		"ccp2 set relay \r\n"
	};
	...
	if (argc == 2) {
		if(argv[1][0] == 'f') {
			...
			pdi_startsession();
			memset(ccp2_fault_buf,0,sizeof(ccp2_fault_buf)); 	
			if (ccp2_GetFault(ccp2_fault_buf, &num_fault))
				printf("##ERROR##\n");
			else {
				printf("##OK##\n");
				...
				for (i = 0; i < num_fault*4; i += 4)
					printf("0x%02x,0x%02x,0x%02x,0x%02x\n", ccp2_fault_buf[i]&0xff,
					ccp2_fault_buf[i+1]&0xff,ccp2_fault_buf[i+2]&0xff,ccp2_fault_buf[i+3]&0xff);
			}
			pdi_IGN_off();
		}
...
const cmd_t cmd_ccp2 = {"ccp2", cmd_ccp2_func, "ccp2 cmd i/f"};
DECLARE_SHELL_CMD(cmd_ccp2)
#endif