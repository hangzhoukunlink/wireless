 ...
 *the length of hzhrcode must larger one byte than the SN: this one time, Max3232 5V is forget, 12V is forget too
 *SN is the hzhrcode!!!!
 *add filter in usdt.c 
 *add gpio config in drv.c and usdt.c like CONFIG_PDI_hzh_gujiamao 
...
//pdi hzh can msg
static const can_msg_t hzh_clrdtc_msg =		{ID, 8, {0x04, 0x14, 0xFF, 0xFF, 0xFF, 0, 0, 0}, 0};
//static const can_msg_t hzh_resis =			{0x607, 8, {0x03, 0x22, 0xfe, 0x80, 0, 0, 0, 0}, 0};
#if PDI_DEBUG
static const can_msg_t hzh_errcode_msg =		{ID, 8, {0x03, 0x19, 0xfd, 0x39, 0, 0, 0, 0}, 0};
static const can_msg_t hzh_getsn_msg =			{ID, 8, {0x03, 0x22, 0xfe, 0x8d, 0, 0, 0, 0}, 0};
static const can_msg_t hzh_part_msg =			{0x7D2, 8, {0x03, 0x22, 0xF1, 0x00, 0x55, 0x55, 0x55, 0x55}, 0};
#endif
static const can_msg_t hzh_reqseed_msg =		{ID, 8, {0x02, 0x27, 0x61, 0, 0, 0, 0, 0}, 0};
static const can_msg_t hzh_start_msg =			{ID, 8, {0x02, 0x10, 0x60, 0, 0, 0, 0, 0}, 0};
static can_msg_t hzh_msg_buf[32];		// for multi frame buffer
static char hzh_data_buf[256];			// data buffer
static char hzh_fault_buf[64];			// fault buffer
...
// get data from ECU
static int hzh_GetCID(char sid,short cid, char *data);
static int hzh_GetFault(char *data, int *pnum_fault);
...
int pdi_startsession(void)
{
	...
	do {
		usdt_GetDiagFirstFrame(&hzh_start_msg, 1, NULL, &msg, &msg_len);		//start session, why twice?	
		try_time_start --;
...
#if PDI_DEBUG
	printf("111111\r\n");
	can_msg_print(&hzh_start_msg,"\n");
	can_msg_print(&msg, "\n");
#endif
	int try_time_reqseed = 5;
	do {
		usdt_GetDiagFirstFrame(&hzh_reqseed_msg, 1, NULL, &msg, &msg_len);		//req seed
		try_time_reqseed --;
	...
	/*get serial number*/
	printf("\nSN Code:\n");
	usdt_GetDiagFirstFrame(&hzh_getsn_msg, 1, NULL, hzh_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(hzh_msg_buf, msg_len);
	for (i = 0; i < msg_len; i ++)
		can_msg_print(hzh_msg_buf + i, "\n");

	/*get error code*/
	printf("\nError Code:\n");
	usdt_GetDiagFirstFrame(&hzh_errcode_msg, 1, NULL, hzh_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(hzh_msg_buf, msg_len);
	for (i = 0; i < msg_len; i++)
		can_msg_print(hzh_msg_buf + i, "\n");
	/*tester point*/
	printf("\nPart:\n");
	usdt_GetDiagFirstFrame(&hzh_getpart_msg, 1, NULL, hzh_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(hzh_msg_buf, msg_len);
	for (i = 0; i < msg_len; i++)
		can_msg_print(hzh_msg_buf + i, "\n");

	if (hzh_GetFault(hzh_fault_buf, &num_fault))
		printf("##ERROR##\n");
	else {
		printf("##OK##\n");
		printf("num of fault is: %d\n", num_fault);
		for (i = 0; i < num_fault*2; i += 2)
			printf("0x%02x, 0x%02x\n", hzh_fault_buf[i]&0xff, hzh_fault_buf[i+1]&0xff);
	}
	/*clear all*/
	printf("\nClear all:\n");
	if (usdt_GetDiagFirstFrame(&hzh_clrdtc_msg, 1, NULL, &msg, &msg_len))	//req seed
		return 1;
	can_msg_print(&msg, "\n");
#endif
#endif
...
static int hzh_GetCID(char sid,short cid, char *data)
{
	...
	if (msg_len > 1) {
		hzh_msg_buf[0] = msg_res;
		if(usdt_GetDiagLeftFrame(hzh_msg_buf, msg_len))
			return 1;
	}
	if (msg_len == 1) {
		...
		data += 3;
		for (i = 1; i < msg_len; i++) {
			memcpy(data, (hzh_msg_buf + i)->data + 1, 7);
			data += 7;
		}
	}
	return 0;
...
// return 1 : no date
// retrun 0 : ok
static int hzh_read_barcode() 
{
	char read_bcode[Length_barcode + 1];
	int try_times = 5;
	while (hzh_GetCID(0x22,0xf18c, hzh_data_buf)) {
		try_times --;
		if (try_times < 0)
			return 1;
	}
	memcpy(read_bcode, hzh_data_buf, Length_barcode);
...
static int hzh_check_barcode()
{
	...
	printf("##END##\n");
	if(position == LEFT) {
		if (memcmp(read_bcode, bcodea, Length_barcode))  //maybe Length_hzhrcode - 1 ??
			return 1;
	}
	else {
...
//return 1 : no date
//return 0 : ok
static int hzh_read_sn()
{
	char read_sn[Length_SN + 1];
	int try_times = 5;
	memset(hzh_data_buf,0,sizeof(hzh_data_buf));
	while(hzh_GetCID(0x22,0xff8c,hzh_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	}
	hzh_data_buf[Length_SN] = '\0';
	memcpy(read_sn,hzh_data_buf,sizeof(read_sn));
	printf("SN:%s\r\n",read_sn);
	return 0;
}
/* equal Serial Number */
...
//return 1 : no date
//return 0 : ok
static int hzh_read_cpn()
{
	char read_cpn[length_cpn + 1];
	int try_times = 5;
	memset(hzh_data_buf,0,sizeof(hzh_data_buf));
	while(hzh_GetCID(0x22,0xf187,hzh_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	}
	hzh_data_buf[length_cpn] = '\0';
	memcpy(read_cpn, hzh_data_buf, sizeof(read_cpn));
	printf("CPN:%s\r\n", read_cpn);
	return 0;
}
/* equal Serial Number */
...
static int hzh_read_softversion()
{
	char read_softversion[11 + 1];  // 12 means length
	int try_times = 5;
	memset(hzh_data_buf,0,sizeof(hzh_data_buf));
	while(hzh_GetCID(0x22,0xfda5,hzh_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	...
	hzh_data_buf[11] = '\0';
	memcpy(read_softversion,hzh_data_buf,sizeof(read_softversion));
	printf("%s\r\n",read_softversion);
	return 0;
}
static int hzh_check_softversion()
{
	char read_softversion[11 + 1];  // 12 means length
	int try_times = 5;
	memset(hzh_data_buf,0,sizeof(hzh_data_buf));
...
/*checking Resis*/
/*static int hzh_check_resis()
{
	int msg_len;
	can_msg_t msg;
	printf("##START##EC-Checking resis...##END##\n");
	if (usdt_GetDiagFirstFrame(&hzh_resis, 1, NULL, &msg, &msg_len))
		return 1;
	if (msg_len > 1)
	usdt_GetDiagLeftFrame(hzh_msg_buf, msg_len);
...
int pdi_clear_dtc(void)
{
	...
	can_msg_t msg;
	pdi_startsession();
	if (usdt_GetDiagFirstFrame(&hzh_clrdtc_msg, 1, NULL, &msg, &msg_len))
		return 1;
	if (msg.data[1] != (hzh_clrdtc_msg.data[1] + 0x40))	// TBD By Me
		return 1;
	return 0;
}
...
{
	int i, result = 0;
	if (hzh_GetCID(0x22,0xfd80, data))
		return 1;
...
** compare hzhrcode from ECU and scaner  **
...
ERROR_E Testbarcode(void)
{
	...
	for (rate = 5; rate <= 17; rate ++) {
		...
		printf("##START##STATUS-");		// in such format, so the data can be displayed in processhzhr
		sprintf(processbar, "%d", rate);
		printf("%s", processbar);
		printf("##END##\n");
	}
	pdi_startsession();                 // startsession before getting message
	//if (hzh_check_barcode()) {
	//	error_result = ERROR_BW;
	//	return error_result;
	//}
	printf("####EC-      Checking hzhrcode Done...##END##\n");
	return ERROR_NO;
}
ERROR_E TestSN(void)
{
...
ERROR_E TestECU(void)
{
	...
	memset(hzh_fault_buf,0,sizeof(hzh_fault_buf)); // clear this buffer, if not, you will see the bug
	int try_times = 0;
	int result = 0;
	int num_fault = 0;
	...
	*******************************************************************/
	pdi_startsession();
	do{
		result = hzh_GetFault(hzh_fault_buf,&num_fault);
		if(++try_times == 4) {
			error_result = ERROR_CW;
			return error_result;
		}
	...
	if(!num_fault) {
		...
		for (int i = 0; i < num_fault * 3; i += 3)  // according to size of error code , sometimes 3 bytes 
			printf("0x%02x, 0x%02x, 0x%02x\n", hzh_fault_buf[i] & 0xff, hzh_fault_buf[i + 1] & 0xff, hzh_fault_buf[i + 2] & 0xff);
		printf("##END##\n");
		if(pdi_error_check(hzh_fault_buf, &num_fault, autoliv)) { // check whether the error code is allowable
			...
			autoliv_send(&pdi_autoliv,"ERROR\r\n ");  //new protocol add by hyf at shanghai 2014/5/15  ' 'for the upper's convinence
				for(int i = 0; i < num_fault * 3; i++){   //send errorcode to up
					//autoliv_send_char(&pdi_autoliv, hzh_fault_buf[i]);
					char a , b;
					a = hzh_fault_buf[i] >> 4;
					b = hzh_fault_buf[i] & 0x0f;
					if((a >= 0) && (a <= 9)) {
						a += '0';
					} else if((a >= 10) && (a <= 15)) {
						a = a - 10 + 'a';
...
{	
	...
	do {                                // if no extern CAN
		pdi_mdelay(1000);
	} while(!usdt_GetDiagFirstFrame(&hzh_start_msg, 1, NULL, &msg, &msg_len));
}
/******************************
** change the status **********
** unlock the fixture *********
...
{
	memset(autoliv_buf,0,sizeof(autoliv_buf));   // Glohzhl variable attention!!!
	time_t over_time;
...
// add by jiamao.gu in Shanghai 2013.10.28
// hzhrcode sended after teststop in about three seconds, so need to wait 
...
void Receive_from_Up(time_t over_time)
{
	while(time_left(over_time) > 0) {
		if(SN[0] != 0) { // All the receive data put in autoliv_buf			
			printf("Receive other hzhrcode and SN success !\r\n");
			return ;
		}
		sys_update();
	}
...
#if 1
static int cmd_hzh_func(int argc, char *argv[])
{
	int num_fault, i;

	const char *usage = {
		"hzh , usage:\r\n"
		"hzh fault		read the error code\r\n"
		"hzh start     test the hzh \r\n"
		"hzh sn          read the sn\r\n"
		"hzh barcode     read barcode inner\r\n"
		"hzh softversion \r\n "
		"hzh IMU    read IMU can message\r\n"
		"hzh clear       clear the history error\r\n"
		"hzh lock x      lock the fixture x\r\n"
		"hzh unlock x    unlock the fixture x\r\n"
		"hzh printf relay file \r\n"
		"hzh get position  \r\n"
		"hzh set position x  left or right\r\n"
		"hzh set relay \r\n"
	};
	...
	if (argc == 2) {
		if(argv[1][0] == 'f') {
			...
			memset(hzh_fault_buf, 0, sizeof(hzh_fault_buf));
			pdi_startsession();
			if (hzh_GetFault(hzh_fault_buf, &num_fault))
				printf("##ERROR##\n");
			else {
				...
				for (i = 0; i < num_fault * 3; i += 3)
					printf("0x%02x, 0x%02x, 0x%02x\n", hzh_fault_buf[i] & 0xff,
					hzh_fault_buf[i + 1] & 0xff, hzh_fault_buf[i + 2] & 0xff);
			}
			pdi_IGN_off();
		}
...
const cmd_t cmd_hzh = {"hzh", cmd_hzh_func, "hzh cmd i/f"};
DECLARE_SHELL_CMD(cmd_hzh)
#endif