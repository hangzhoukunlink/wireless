 ...
 *the length of b12lrcode must larger one byte than the SN: this one time, Max3232 5V is forget, 12V is forget too
 *SN is the b12lrcode!!!!
 *add filter in usdt.c 
 *add gpio config in drv.c and usdt.c like CONFIG_PDI_b12l_gujiamao 
...
#define Length_SN      16		 // TBD for different Product
#define ID 0x752
...
//pdi b12l can msg
static const can_msg_t b12l_clrdtc_msg =		{ID, 8, {0x03, 0x2E, 0xFD, 0xFF, 0x0, 0, 0, 0}, 0};
//static const can_msg_t b12l_resis =			{0x607, 8, {0x03, 0x22, 0xfe, 0x80, 0, 0, 0, 0}, 0};
#if PDI_DEBUG
static const can_msg_t b12l_errcode_msg =		{ID, 8, {0x03, 0x19, 0xfd, 0x39, 0, 0, 0, 0}, 0};
static const can_msg_t b12l_getsn_msg =			{ID, 8, {0x03, 0x22, 0xfe, 0x8d, 0, 0, 0, 0}, 0};
static const can_msg_t b12l_part_msg =			{0x7D2, 8, {0x03, 0x22, 0xF1, 0x00, 0x55, 0x55, 0x55, 0x55}, 0};
#endif
static const can_msg_t b12l_reqseed_msg =		{ID, 8, {0x02, 0x27, 0x61, 0, 0, 0, 0, 0}, 0};
static const can_msg_t b12l_start_msg =			{ID, 8, {0x02, 0x10, 0x61, 0, 0, 0, 0, 0}, 0};
static can_msg_t b12l_msg_buf[32];		// for multi frame buffer
static char b12l_data_buf[256];			// data buffer
static char b12l_fault_buf[64];			// fault buffer
...
// get data from ECU
static int b12l_GetCID(char sid,short cid, char *data);
static int b12l_GetFault(char *data, int *pnum_fault);
...
int pdi_startsession(void)
{
	...
	do {
		usdt_GetDiagFirstFrame(&b12l_start_msg, 1, NULL, &msg, &msg_len);		//start session, why twice?	
		try_time_start --;
...
#if PDI_DEBUG
	printf("111111\r\n");
	can_msg_print(&b12l_start_msg,"\n");
	can_msg_print(&msg, "\n");
#endif
	int try_time_reqseed = 5;
	do {
		usdt_GetDiagFirstFrame(&b12l_reqseed_msg, 1, NULL, &msg, &msg_len);		//req seed
		try_time_reqseed --;
...
#if PDI_DEBUG
	printf("222222\r\n");
	can_msg_print(&b12l_reqseed_msg,"\n");
	can_msg_print(&msg, "\n");
#endif
#if 0
	...
	/*get serial number*/
	printf("\nSN Code:\n");
	usdt_GetDiagFirstFrame(&b12l_getsn_msg, 1, NULL, b12l_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(b12l_msg_buf, msg_len);
	for (i = 0; i < msg_len; i ++)
		can_msg_print(b12l_msg_buf + i, "\n");

	/*get error code*/
	printf("\nError Code:\n");
	usdt_GetDiagFirstFrame(&b12l_errcode_msg, 1, NULL, b12l_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(b12l_msg_buf, msg_len);
	for (i = 0; i < msg_len; i++)
		can_msg_print(b12l_msg_buf + i, "\n");
	/*tester point*/
	printf("\nPart:\n");
	usdt_GetDiagFirstFrame(&b12l_getpart_msg, 1, NULL, b12l_msg_buf, &msg_len);
	if (msg_len > 1)
		usdt_GetDiagLeftFrame(b12l_msg_buf, msg_len);
	for (i = 0; i < msg_len; i++)
		can_msg_print(b12l_msg_buf + i, "\n");

	if (b12l_GetFault(b12l_fault_buf, &num_fault))
		printf("##ERROR##\n");
	else {
		printf("##OK##\n");
		printf("num of fault is: %d\n", num_fault);
		for (i = 0; i < num_fault*2; i += 2)
			printf("0x%02x, 0x%02x\n", b12l_fault_buf[i]&0xff, b12l_fault_buf[i+1]&0xff);
	}
	/*clear all*/
	printf("\nClear all:\n");
	if (usdt_GetDiagFirstFrame(&b12l_clrdtc_msg, 1, NULL, &msg, &msg_len))	//req seed
		return 1;
	can_msg_print(&msg, "\n");
#endif
#endif
...
static int b12l_GetCID(char sid,short cid, char *data)
{
	...
	if (msg_len > 1) {
		b12l_msg_buf[0] = msg_res;
		if(usdt_GetDiagLeftFrame(b12l_msg_buf, msg_len))
			return 1;
	}
	if (msg_len == 1) {
		...
		data += 3;
		for (i = 1; i < msg_len; i++) {
			memcpy(data, (b12l_msg_buf + i)->data + 1, 7);
			data += 7;
		}
	}
	return 0;
...
// return 1 : no date
// retrun 0 : ok
static int b12l_read_barcode() 
{
	char read_bcode[Length_barcode + 1];
	int try_times = 5;
	while (b12l_GetCID(0x22,0xfdab, b12l_data_buf)) {
		try_times --;
		if (try_times < 0)
			return 1;
	}
	memcpy(read_bcode, b12l_data_buf, Length_barcode);
...
static int b12l_check_barcode()
{
	...
	printf("##END##\n");
	if(position == LEFT) {
		if (memcmp(read_bcode, bcodea, Length_barcode))  //maybe Length_b12lrcode - 1 ??
			return 1;
	}
	else {
...
//return 1 : no date
//return 0 : ok
static int b12l_read_sn()
{
	char read_sn[Length_SN + 1];
	int try_times = 5;
	memset(b12l_data_buf,0,sizeof(b12l_data_buf));
	while(b12l_GetCID(0x22,0xfdaa,b12l_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	}
	b12l_data_buf[Length_SN] = '\0';
	memcpy(read_sn,b12l_data_buf,sizeof(read_sn));
	printf("SN:%s\r\n",read_sn);
	return 0;
}
/* equal Serial Number */
...
static int b12l_read_softversion()
{
	char read_softversion[11 + 1];  // 12 means length
	int try_times = 5;
	memset(b12l_data_buf,0,sizeof(b12l_data_buf));
	while(b12l_GetCID(0x22,0xfda5,b12l_data_buf)) {
		try_times --;
		if(try_times < 0) {
			return 1;
		}
	...
	b12l_data_buf[11] = '\0';
	memcpy(read_softversion,b12l_data_buf,sizeof(read_softversion));
	printf("%s\r\n",read_softversion);
	return 0;
}
static int b12l_check_softversion()
{
	char read_softversion[11 + 1];  // 12 means length
	int try_times = 5;
	memset(b12l_data_buf,0,sizeof(b12l_data_buf));
...
/*checking Resis*/
/*static int b12l_check_resis()
{
	int msg_len;
	can_msg_t msg;
	printf("##START##EC-Checking resis...##END##\n");
	if (usdt_GetDiagFirstFrame(&b12l_resis, 1, NULL, &msg, &msg_len))
		return 1;
	if (msg_len > 1)
	usdt_GetDiagLeftFrame(b12l_msg_buf, msg_len);
...
int pdi_clear_dtc(void)
{
	...
	can_msg_t msg;
	pdi_startsession();
	if (usdt_GetDiagFirstFrame(&b12l_clrdtc_msg, 1, NULL, &msg, &msg_len))
		return 1;
	if (msg.data[1] != (b12l_clrdtc_msg.data[1] + 0x40))	// TBD By Me
		return 1;
	return 0;
}
...
{
	int i, result = 0;
	if (b12l_GetCID(0x22,0xfd80, data))
		return 1;
...
** compare b12lrcode from ECU and scaner  **
...
ERROR_E Testbarcode(void)
{
	...
	for (rate = 5; rate <= 17; rate ++) {
		...
		printf("##START##STATUS-");		// in such format, so the data can be displayed in processb12lr
		sprintf(processbar, "%d", rate);
		printf("%s", processbar);
		printf("##END##\n");
	}
	pdi_startsession();                 // startsession before getting message
	if (b12l_check_barcode()) {
		error_result = ERROR_BW;
		return error_result;
	}
	printf("####EC-      Checking b12lrcode Done...##END##\n");
	return ERROR_NO;
}
ERROR_E TestSN(void)
{
...
ERROR_E TestECU(void)
{
	...
	memset(b12l_fault_buf,0,sizeof(b12l_fault_buf)); // clear this buffer, if not, you will see the bug
	int try_times = 0;
	int result = 0;
	int num_fault = 0;
	...
	*******************************************************************/
	pdi_startsession();
	do{
		result = b12l_GetFault(b12l_fault_buf,&num_fault);
		if(++try_times == 4) {
			error_result = ERROR_CW;
			return error_result;
		}
	...
	if(!num_fault) {
		...
		for (int i = 0; i < num_fault * 3; i += 3)  // according to size of error code , sometimes 3 bytes 
			printf("0x%02x, 0x%02x, 0x%02x\n", b12l_fault_buf[i]&0xff,b12l_fault_buf[i+1]&0xff,b12l_fault_buf[i+2]&0xff);
		printf("##END##\n");
		if(pdi_error_check(b12l_fault_buf, &num_fault, autoliv)) { // check whether the error code is allowable
			...
			autoliv_send(&pdi_autoliv,"ERROR\r\n ");  //new protocol add by hyf at shanghai 2014/5/15  ' 'for the upper's convinence
				for(int i = 0; i < num_fault * 3; i++){   //send errorcode to up
					//autoliv_send_char(&pdi_autoliv, b12l_fault_buf[i]);
					char a , b;
					a = b12l_fault_buf[i] >> 4;
					b = b12l_fault_buf[i] & 0x0f;
					if((a >= 0) && (a <= 9)) {
						a += '0';
					} else if((a >= 10) && (a <= 15)) {
						a = a - 10 + 'a';
...
{	
	...
	do {                                // if no extern CAN
		pdi_mdelay(1000);
	} while(!usdt_GetDiagFirstFrame(&b12l_start_msg, 1, NULL, &msg, &msg_len));
}
/******************************
** change the status **********
** unlock the fixture *********
...
{
	memset(autoliv_buf,0,sizeof(autoliv_buf));   // Glob12ll variable attention!!!
	time_t over_time;
...
void Step0(void)
{
	autoliv_send(&pdi_autoliv,"ECUPDIIDLT003\r\n");  // Send to Up
	printf("%s\r\n","ECUPDILT003\r\n");
}
/***********************************************************************************
...
// add by jiamao.gu in Shanghai 2013.10.28
// b12lrcode sended after teststop in about three seconds, so need to wait 
...
void Receive_from_Up(time_t over_time)
{
	while(time_left(over_time) > 0) {
		if(SN[0] != 0) { // All the receive data put in autoliv_buf			
			printf("Receive other b12lrcode and SN success !\r\n");
			return ;
		}
		sys_update();
	}
...
#if 1
static int cmd_b12l_func(int argc, char *argv[])
{
	int num_fault, i;

	const char *usage = {
		"b12l , usage:\r\n"
		"b12l fault		read the error code\r\n"
		"b12l start     test the b12l \r\n"
		"b12l sn          read the sn\r\n"
		"b12l barcode     read barcode inner\r\n"
		"b12l softversion \r\n "
		"b12l IMU    read IMU can message\r\n"
		"b12l clear       clear the history error\r\n"
		"b12l lock x      lock the fixture x\r\n"
		"b12l unlock x    unlock the fixture x\r\n"
		"b12l printf relay file \r\n"
		"b12l get position  \r\n"
		"b12l set position x  left or right\r\n"
		"b12l set relay \r\n"
	};
		...
		if(argv[1][0] == 'f') {
			...
			if (b12l_GetFault(b12l_fault_buf, &num_fault))
				...
				for (i = 0; i < num_fault*3; i += 3)
					printf("0x%02x,0x%02x,0x%02x\n", b12l_fault_buf[i]&0xff,
					b12l_fault_buf[i+1]&0xff, b12l_fault_buf[i+2]&0xff);
			}
			pdi_IGN_off();
		}
...
const cmd_t cmd_b12l = {"b12l", cmd_b12l_func, "b12l cmd i/f"};
DECLARE_SHELL_CMD(cmd_b12l)
#endif