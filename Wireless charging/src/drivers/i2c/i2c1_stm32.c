...
static int i2c_WriteByte(unsigned char chip, unsigned addr, int alen, unsigned char *buffer)
{
	...
	I2C_Send7bitAddress(I2C1, chip, I2C_Direction_Transmitter);
	...
	if (alen == 1) {
		...
		I2C_SendData(I2C1, (unsigned char)((addr & 0xFF00) >> 8));
		/* Test on EV8 and clear it */
		while(!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
  		/* Send the EEPROM's internal address to write to : LSB of the address */
		I2C_SendData(I2C1, (unsigned char)(addr & 0x00FF));
	} else
		return 1;
...
static int i2c_ReadBuffer(unsigned char chip, unsigned addr, int alen, unsigned char *buffer, int len)
{
	...
	I2C_GenerateSTART(I2C1, ENABLE);
	while(!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT));
	I2C_Send7bitAddress(I2C1, chip, I2C_Direction_Receiver);
